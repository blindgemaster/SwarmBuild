...
async function startAgentInteractive(api, role, jobInfo, isPlanning = false, WORKSPACE) {
    ...
    if (isPlanning && role === 'lead') {
        ...
        if (isSoloLead) {
            prompt = `Read AGENT_PROMPT.md. You are the SOLE agent on this job. Use swarmbuild_create_tasks to create all tasks. Assign ALL tasks to assigned_role = "lead" since you are the only contributor. DO NOT write code yourself during planning.`;
        } else {
            prompt = `Read AGENT_PROMPT.md. Use swarmbuild_create_tasks to create tasks for the team. YOU MUST map every task to one of these specific assigned_roles: [${availableRoles}]. DO NOT invent roles like "developer", use ONLY the provided roles. DO NOT write code yourself.`;
        }
    } else if (isSoloLead) {
